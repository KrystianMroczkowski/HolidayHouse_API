...
namespace HolidayHouse_HouseAPI.Repository
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        public async Task<List<T>> GetAllAsync(Expression<Func<T, bool>>? filter = null, bool tracked = true, string? includeProperties = null, int pageSize = 0, int pageNumber = 1)
        {
            IQueryable<T> query = dbSet;
